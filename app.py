...
st.markdown("""
    ...
    .stMarkdown, .stTextArea, .stTextInput, .stRadio, .stSelectbox, p, 
    h1, h2, h3, label {
        direction: rtl !important;
        text-align: right !important;
    }
    .timer-badge {
        background-color: #e11d48; color: white; padding: 12px;
        border-radius: 10px;
    ...
    .stButton > button {
        border-radius: 10px; background-color: #1E3A8A !important;
        color: white !important;
        width: 100%; height: 45px; font-weight: bold;
    }
    .correct-box { background-color: #dcfce7; padding: 10px;
        border-radius: 5px; border-right: 5px solid #16a34a; margin-top: 5px; }
    .wrong-box { background-color: #fee2e2; padding: 10px; border-radius: 5px;
        border-right: 5px solid #dc2626; margin-top: 5px; }
    </style>
...
def save_to_permanent_excel(student_info, exam_details, final_score):
    """حفظ النتائج الإجمالية في ملف إكسل بشكل دائم لدى سيرفر المعلم"""
...
def calculate_smart_score(student_ans, model_ans):
    """التقييم الذكي للإجابات المقالية"""
...
def generate_smart_exam(data, full_text, difficulty, default_time):
    ...
    for s in sentences:
        ...
        if q_type == "اختياري":
            ...
            all_pool.append({
                "q": f"هل العبارة التالية صحيحة: {s[:100]}؟",
                "a": "صح",
                "type": "صح_خطأ",
                "time": default_time,
                "options": ["صح", "خطأ"]
    ...
    # خلط الاختيارات وسحب العدد المطلوب بدقة منعاً لأي فقدان
    random.shuffle(all_pool)
    qa_pairs = all_pool[:num_questions]

    # في حال كان المنهج قصيراً جداً، نقوم بتوليد الحد الأدنى يدوياً لضمان عدم حدوث خطأ برمجي
    if len(qa_pairs) < num_questions:
        fallback_count = num_questions - len(qa_pairs)
        for i in range(fallback_count):
            qa_pairs.append({
                "q": f"سؤال إثرائي عام رقم {i + 1} حول المنهج المرفوع:",
                "a": "مراجعة النص الأصلي",
                "type": "مقالي",
                "time": default_time,
                "options": []
            })
...
for key in ['qa_pairs', 'exam_active', 'is_authenticated', 'student_step', 'st_ans_list', 'curr', 'q_start',
            'final_score_str', 'has_ballooned']:
    if key not in st.session_state:
        st.session_state[key] = [] if 'list' in key or 'qa' in key else (
            False if 'active' in key or 'auth' in key or 'ballooned' in key else (0 if key == 'curr' else 'login'))
    ...
    if not st.session_state.is_authenticated:
        ...
        if menu == "إعداد الاختبار":
            ...
            if st.session_state.qa_pairs:
                st.info(f"📊 تم توليد {len(st.session_state.qa_pairs)} سؤالاً بناءً على صعوبة النظام الاختيارية.")
                ...
                if st.button("🚀 تفعيل الاختبار للطلاب", use_container_width=True):
                    ...
                    st.balloons()  # تظهر البالونات هنا دائماً عند اعتماد المسؤول للاختبار
            ...
            if os.path.exists("permanent_results.xlsx"):
                ...
                with open("permanent_results.xlsx", "rb") as f:
                    st.download_button("📥 تحميل سجل النتائج الشامل (Excel)", f, "results.xlsx",
...
with tab_student:
    if not st.session_state.exam_active:
        ...
        if st.session_state.student_step == 'login':
            ...
            if st.button("بدء الاختبار 🏁") and name and id_st:
                st.session_state.update(
                    {"st_info": {"name": name, "id": id_st}, "st_ans_list": [], "curr": 0, "q_start": time.time(),
                     "student_step": 'exam', "has_ballooned": False})
                st.rerun()
            ...
            # استخراج الرقم الصافي للنسبة المئوية لمعرفة حالة النجاح (أكبر من أو يساوي 50)
            try:
                score_value = float(st.session_state.final_score_str.replace('%', ''))
            except:
                score_value = 0.0

            # إطلاق البالونات فقط إذا كان الطالب ناجحاً (معدله 50% فما فوق) ولم تظهر مسبقاً في هذه الجلسة
            if score_value >= 50.0 and not st.session_state.has_ballooned:
                st.balloons()
                st.session_state.has_ballooned = True

            if score_value >= 50.0:
                st.success(f"🎊 مبارك النجاح يا {st.session_state.st_info['name']}!")
            else:
                st.error(f"نتمنى لك التوفيق في المرة القادمة يا {st.session_state.st_info['name']}")

            st.metric("درجتك النهائية هي:", st.session_state.final_score_str)

            # ======================================================================
            # إنشاء وتحميل ملف Excel مخصص ومحدث للطالب بالكامل (من اليمين لليسار)
            # ======================================================================
            flat_results = []
            for i, item in enumerate(st.session_state.st_ans_list):
                flat_results.append({
                    "رقم السؤال": i + 1,
                    "نص السؤال": item['q_text'],
                    "إجابة الطالب": item['student_ans'],
                    "الإجابة النموذجية": item['correct_ans'],
                    "الدرجة": f"{item['q_score']}%"
                })

            df_excel = pd.DataFrame(flat_results)
            output_io = io.BytesIO()

            with pd.ExcelWriter(output_io, engine='xlsxwriter') as writer:
                df_excel.to_excel(writer, index=False, sheet_name='النتائج', startrow=4)
                workbook = writer.book
                worksheet = writer.sheets['النتائج']
                worksheet.right_to_left()  # محاذاة الملف لليمين ليتطابق مع اللغة العربية

                # ترويسة مخصصة بشعار الجامعة الأسمرية وبيانات الطالب
                header_format = workbook.add_format({'bold': True, 'font_color': '#1E3A8A', 'font_size': 14})
                worksheet.write('A1', f"الاسم: {st.session_state.st_info['name']}", header_format)
                worksheet.write('A2', f"رقم القيد: {st.session_state.st_info['id']}", header_format)
                worksheet.write('A3', f"المعدل النهائي: {st.session_state.final_score_str}", header_format)
                worksheet.set_column('A:E', 25)

            st.divider()
            col_dl, col_main_btn = st.columns([1, 1])

            with col_dl:
                st.download_button(
                    label="📥 تحميل تقرير إجاباتك (Excel)",
                    data=output_io.getvalue(),
                    file_name=f"Result_{st.session_state.st_info['id']}.xlsx",
...
# الباركود والشريط الجانبي (Sidebar)
with st.sidebar:
